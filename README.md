...
- **`Vitrine.jsx`**: Página principal com listagem dinâmica, carregamento via esqueletos visuais (_skeletons_), tratamento de erros e filtros.
...
## 🚀 Extensões e Diferenciais Implementados

Conforme os requisitos da tarefa acadêmica, este projeto conta com as seguintes extensões implementadas e funcionais:

1. **Deploy da Aplicação:** O projeto encontra-se online e acessível publicamente através do link: [🔗 Acessar Vitrine Alegre](https://vitrine-alegre-flax.vercel.app/)
2. **Carrinho e Dados Persistentes (`localStorage`):** Toda a gestão do carrinho de compras, dados de perfil ("Minha Conta") e histórico de pedidos ("Meus Pedidos") é sincronizada e persistida localmente no navegador do usuário.
3. **Gerenciamento de Requisições (`AbortController`):** Utilizado nos serviços de API para cancelar requisições pendentes de forma otimizada e evitar vazamentos de memória ou conflitos de setState.
